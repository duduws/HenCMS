<script>
	var horario = {
        desmarcar:function(dia,hora){
        	$.ajax({
                type:'GET',
                url:'pagina/<?php echo $url; ?>/desmarcar/'+dia+'/'+hora,
...
if(isset($_GET['tipo']) && $_GET['tipo'] == 'marcar'){
	$dia_m = $_GET['dia'];
	$hora_m = $_GET['hora'];
	$marcado = $pdo->query("SELECT * FROM aa_promocao WHERE user_id = '0' AND dia = '".$dia_m."' AND hora = '".$hora_m."'")->rowCount();
	if($marcado > 0){
		$horario_m = $pdo->query("UPDATE aa_promocao SET user_id='".$aa_data['id']."' WHERE dia='$dia_m' AND hora='$hora_m'");
		echo 1;
	}
}else if(isset($_GET['tipo']) && $_GET['tipo'] == 'desmarcar'){
	$dia_m = $_GET['dia'];
	$hora_m = $_GET['hora'];
	$marcado = $pdo->query("SELECT * FROM aa_promocao WHERE user_id != '0' AND dia = '".$dia_m."' AND hora = '".$hora_m."'")->rowCount();
	if($marcado > 0){
		$desmarcar_horario = $pdo->query("UPDATE aa_promocao SET user_id='0' WHERE dia='".$dia_m."' AND hora='".$hora_m."'");
		echo 1;
	}
}
...
<?php
	$horarios = $pdo->query("SELECT * FROM aa_promocao WHERE dia='$hor_dia'");
	...
	if($locutor == 'Nenhum'){
		$marcar = '<a style="color: #888">Marcar Horário</a>';
	}else{
		$marcar = '<a style="cursor: pointer" onclick="horario.desmarcar('.$hor_dia.','.$ver['hora'].')">Desmarcar Horário</a>';
	}
?>
