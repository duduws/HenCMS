...
<?php
	if(isset($_GET['tipo']) && $_GET['tipo'] == 'criar'){
        if($_POST){ 
            ...
            $foto = $_FILES['imagem'];
            $texto = $_POST['texto'];
            if(empty($titulo) || empty($descricao) || empty($data_eve) || empty($foto["name"]) || empty($texto)){
                ...
                if(@!eregi("^image\/(pjpeg|jpeg|png|gif|bmp)$", $foto["type"])){
                    ...
                    if($caminho_imagem){
                        $inserir = $pdo->prepare("INSERT INTO noticias(titulo, descricao, categoria, imagem, autor, revisado, revisador time, status, fixo, url, texto, visualizacao, evento, evento_time) VALUES(:titulo, :descricao, '0', :imagem, :autor, 'false', 'Nenhum', '".$time."', 'false', 'false', :url, :texto, '0', 'true', :data)");
...
<form method="POST" enctype="multipart/form-data" style="width: 100%" autocomplete="off">
    ...
    Data do Evento:<br>
    <input type="text" id="datepicker" class="text" name="data"><br><br><br>
    Imagem:<br>
...
<?php
	    ...
	    $quantidade = 20;
        $registros = $pdo->query("SELECT * FROM noticias WHERE evento='true' AND autor='".$aa_data['usuario']."'")->rowCount();
        $pagina     = (isset ($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
        $inicio     = ($quantidade * $pagina) - $quantidade;
        $totalPagina = ceil($registros/$quantidade);
?>
<a href="pagina/<?php echo $url;?>/criar"><input type="button" class="button" value="Criar Evento"></a>
<table width="100%" style="margin: 10px 0 0 0; float:left">
	...
	<?php
    $i = 1;
    $sql = $pdo->query("SELECT * FROM noticias WHERE evento='true' AND autor='".$aa_data['usuario']."' ORDER BY id DESC LIMIT $inicio, $quantidade");
