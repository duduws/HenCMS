...
if(isset($_GET['tipo']) && $_GET['tipo'] == 'apagar'){
	$id = (int) $_GET['id'];
	$delete = $pdo->query("DELETE FROM noticias_comentarios WHERE id='$id'");
    echo 1;
}else{
?>
<table width="100%" style="margin: 10px 0 0 0; float:left">
    ...
    <?php
	$i = 1;
    $sql = $pdo->query("SELECT * FROM noticias_comentarios ORDER BY id DESC");
	while($ver = $sql->fetch(PDO::FETCH_ASSOC)){
		$css = $i%2==0 ? '' : 'background: #EEE;';
        $artigo = $pdo->query("SELECT * FROM noticias WHERE id='".$ver['noticia_id']."'")->fetch(PDO::FETCH_ASSOC);
    ?>
