...
if(isset($_GET['tipo']) && $_GET['tipo'] == 'apagar'){
	$id = (int) $_GET['id'];
	$delete = $pdo->query("DELETE FROM topicos_cat WHERE id='$id'");
    echo 1;
}else{
	if($_POST){
        ...
        if(empty($categoria)){
    		...
    		$inserir = $pdo->prepare("INSERT INTO topicos_cat(categoria) VALUES(:categoria)");
...
<?php
	$i = 1;
    $sql = $pdo->query("SELECT * FROM topicos_cat ORDER BY id DESC");
