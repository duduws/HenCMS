...
function GetLast($a){
		if(!empty($a) || !$a == ''){
			if(is_numeric($a)){
				...
				if($difference <= '59'){ $echo = 'há '. $difference .' segundos'; }
				...
				}else { $echo = 'há '.floor(date('m', $difference)).' meses'; }
				return $echo;
			}else{ return $a; }
...
$chb = mysql_query("SELECT * FROM bans WHERE user_id = '". $myrow['id'] ."' OR ip = '". $myrow['ip_last'] ."' OR machine_id = '". $myrow['machine_id'] ."'");
